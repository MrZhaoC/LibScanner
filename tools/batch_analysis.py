...
def batch_decompress_apk(apk_dir, depress_apk_dir):
    """
    批量解压apk_dir中的apk到depress_apk_dir
    :param apk_dir:
    :param depress_apk_dir:
    :return:
    """
    if not os.path.exists(apk_dir):
        print("apk文件夹不存在...")
...
if __name__ == '__main__':
    dex_path = r'D:\Android-exp\exp-example\haircomb\apk'
    dex_output_path = r'D:\Android-exp\exp-example\haircomb\apk\dcm'

    if not os.path.exists(dex_output_path):
        os.makedirs(dex_output_path)
