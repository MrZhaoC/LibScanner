...
def compare_core_feature_1():
    ...
    for db_tpl_item in db_tpl_feature_info:
        ...
        for cls in clusters:
            ...
            print(len(set(best_match_method_list)), len(cls), len(cls.intersection(set(best_match_method_list))),
                  cls_sim)

            out_data.append(
                [len(set(best_match_method_list)), len(cls), len(cls.intersection(set(best_match_method_list))),
                 cls_sim])  # test

            match_list.append(cls_sim)

        print(db_tpl_name, clusters_num, max(match_list))
        out_data.append([db_tpl_name, clusters_num, max(match_list), ''])  # test
        print('---------end')
    #     if match_list:
    #         res.append([db_tpl_name, max(match_list)])
    #     else:
    #         res.append([db_tpl_name, 0])
    # return res
...
if __name__ == '__main__':
    ...
    # best_match_sim = compare_core_feature_1()
    # for item in best_match_sim:
    #     print('%-80s %s' % (item[0], item[1]))

    output_data = compare_core_feature_1()

    write_file_flag = True
    path = r"G:\2023-09-26.xlsx"
    if write_file_flag:
        write_excel_xlsx(path, "核心特征划分比较结果", output_data)
