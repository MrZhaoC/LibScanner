...
def get_class_names():
    ...
    for d in d_list:
        for full_class_name in d.get_classes():
            clazz_name = full_class_name.name
            end_class_name = str(clazz_name).split('/')[-1]

            if pattern.search(end_class_name) or end_class_name == 'R;':
                continue
            if str(clazz_name).startswith(correct_package_name):
                main_class.append(clazz_name)
            if not str(clazz_name).startswith(correct_package_name):
                library_class.append(clazz_name)
    return main_class, library_class


# 分析直接调用apk_primary_code的library_class
