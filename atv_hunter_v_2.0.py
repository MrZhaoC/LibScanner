...
from analysis.core_feature_analysis.android_r8_core_feature.old.core_feature_cluster import get_current_dex_core_feature_clusters
...
def compare_core_feature_1():
    ...
    for db_tpl_item in db_tpl_feature_info:
        ...
        if match_list:
            res.append([db_tpl_name, clusters_num, max(match_list)])
        else:
            res.append([db_tpl_name, clusters_num, 0])
    return res
...
if __name__ == '__main__':
    ...
    # # print(best_match_sim)
    # for item in best_match_sim:
    #     print('%-80s %-20s %s' % (item[0], item[1], item[2]))
