...
def analysis_method_entry_from_dependency(format_dex_name, target_dex_path, converted_android_dex, default_config,
    ...
    sorted_format_method_keep_rules = []
    # 2024-01-02修改
    for i, file in enumerate(converted_android_dex):
        ...
        if file_ext == '.dex':
            ...
            if method_entry_list:
                ...
                if sorted_format_method_keep_rules:
                    if sorted_format_method_keep_rules == format_method_keep_rules:
                        logger.debug('重复方法入口集合,无需再次生成配置文件 {}'.format(file_name))
                        continue
                else:
    ...
    for config_file in tools.tools.list_all_files(config_output_path):
        ...
        try:
            cmd = 'java -jar  %s ' \
                  ...
                  '--lib E:\\JDK8 %s' \
                  % (
                      r'F:\pythonProject\ATVHunter\libs\r8-3.2.74.jar', shrink_item_path, config_file,
                      input_library_path)
...
def pre_main(dependency_file_path, shrink_dex_path, default_rule_config_path):
